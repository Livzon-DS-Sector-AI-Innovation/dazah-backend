...
业务: 维护计划自动生成定时任务
依赖: app.modules.equipment.service.maintenance_plan (generate_due_work_orders)
"""
...
async def maintenance_plan_loop() -> None:
    ...
    # 检查配置项是否存在
    auto_enabled = getattr(settings, 'MAINTENANCE_PLAN_AUTO_ENABLED', False)
    if not auto_enabled:
        logger.info(
    ...
    while not stop_maintenance_plan_flag.is_set():
        ...
        if not getattr(get_settings(), 'MAINTENANCE_PLAN_AUTO_ENABLED', False):
            logger.debug("维护计划自动生成已关闭，跳过本轮")
            continue

        try:
            # TODO: 导入并调用 generate_due_work_orders 服务
            ...
            #         skipped_count,
            #     )
            logger.info("维护计划自动生成任务运行中（服务层待实现）")
        except Exception:
