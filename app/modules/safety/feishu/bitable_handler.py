...
async def _set_sync_ignore(record_id: str, ttl: int = 30) -> None:
    ...
    try:
        ...
        logger.warning("Failed to set Redis ignore key")
...
async def _create_hazard_from_bitable(
    ...
    try:
        ...
        spawn_task(_send_rectification_notification(item), name="safety.bitable.rectification_notification")
    ...
    try:
        ...
        if new_status and new_status != old_rectification_status:
            ...
            if new_status == "replied":
                ...
                spawn_task(_send_verify_notification(hazard, 1), name="safety.bitable.verify_notification")
            elif new_status == "level1_approved":
                if hl == "general":
                    spawn_task(_send_verify_notification(hazard, 3), name="safety.bitable.verify_notification")
                else:
                    spawn_task(_send_verify_notification(hazard, 2), name="safety.bitable.verify_notification")
...
async def handle_card_action(event: dict) -> dict | None:
    ...
    try:
        ...
        if open_message_id:
            spawn_task(_patch_card_async(open_message_id, updated_card, hazard.hazard_no, level, button_state), name="safety.bitable.patch_card")
        else:
