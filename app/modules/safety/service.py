...
from app.platform.integrations.ai.prompts import SCRIPT_CONFIG
...
class SafetyService:
    ...
    async def _generate_ai_output(
        ...
        """调用 AI 服务生成脚本输出。失败时抛出 AIOutputError。

        优先从数据库 ai_workflow_configs 表读取对应模块的脚本配置，
        fallback 到 prompts.py 的硬编码 SCRIPT_CONFIG。
        """
        # 优先从数据库读取工作流配置
        ...
        if (
            ...
            if db_script and db_script.get("is_enabled", True):
                prompt_template = db_script["prompt_template"]
                expected_keys = db_script.get("expected_keys", [])
                logger.debug("使用数据库工作流配置: 脚本%d - %s", script_number, db_script.get("name"))
            else:
                # DB 中有 workflow 但没有对应脚本 → fallback
                config = SCRIPT_CONFIG[script_number]
                prompt_template = config["prompt"]
                expected_keys = config["expected_keys"]
                logger.debug(
                    "数据库工作流配置中未找到脚本 %d，fallback 到硬编码", script_number
                )
        else:
            ...
            prompt_template = config["prompt"]
            expected_keys = config["expected_keys"]
            logger.debug("无数据库工作流配置，使用硬编码脚本 %d", script_number)
