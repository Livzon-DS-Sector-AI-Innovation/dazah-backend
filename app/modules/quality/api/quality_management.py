...
async def batch_update_deviation_status(data: BatchUpdateStatusRequest, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def confirm_department_status(data: ConfirmProductionStatusRequest, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def get_stopped_departments(week_key: str, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def get_deviation(deviation_id: uuid.UUID, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def create_deviation(data: CreateDeviationRequest, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def update_deviation(deviation_id: uuid.UUID, data: UpdateDeviationRequest, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def delete_deviation(deviation_id: uuid.UUID, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def submit_deviation_for_review(deviation_id: uuid.UUID, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def submit_investigation(deviation_id: uuid.UUID, data: SubmitInvestigationRequest, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def submit_review(deviation_id: uuid.UUID, data: SubmitReviewRequest, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def submit_final_code(deviation_id: uuid.UUID, final_code: str, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def resubmit_deviation(deviation_id: uuid.UUID, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def get_capa_departments(current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def auto_fill_capa_from_deviation(deviation_id: uuid.UUID, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def get_capa(capa_id: uuid.UUID, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def create_capa(data: CreateCapaRequest, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def update_capa(capa_id: uuid.UUID, data: UpdateCapaRequest, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def delete_capa(capa_id: uuid.UUID, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def submit_capa_for_review(capa_id: uuid.UUID, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def resubmit_capa(capa_id: uuid.UUID, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def confirm_capa_execution(capa_id: uuid.UUID, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def upsert_department_contact(data: CreateDepartmentContactRequest, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def delete_department_contact(contact_id: uuid.UUID, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def get_deviation_statistics(current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def get_capa_statistics(current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def delete_attachment_review(review_id: uuid.UUID, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
