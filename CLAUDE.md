...
每个业务代码文件（`api.py`/`service.py`/`repository.py`/`models.py` 等）**定稿后**，必须在文件最顶部 docstring 写"标准档"头部注释，方便快速遍历和排障：

```python
"""
<模块名> - <子功能>
业务: <一句话说清这个文件负责什么业务流程>
依赖: <连接的外部系统/其他模块，如 飞书车辆 Bot(FEISHU_VEHICLE_*)、礼品库存独立DB>
"""
```

规则（防注释腐烂，重要）：
- **只写"会变得慢、机器生不出来"的内容**：业务描述、对外依赖（连了哪些外部系统/模块）。
- **不手写"会变得快"的值**：端口、接口列表、具体配置值一律不写进注释。
  - 要写端口/配置 → 写指针 `见 app/core/config.py`，不复制具体值。
  - 要写接口清单 → 写 `见 /docs`（FastAPI 自动生成），不手抄路由。
