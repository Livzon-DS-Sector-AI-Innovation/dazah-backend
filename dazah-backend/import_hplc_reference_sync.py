...
# Database config
DATABASE_URL = "postgresql://postgres:postgres@localhost:5432/dahzah"
...
def import_hplc_reference():
    ...
    print(f"Header index: {header_to_idx}")
    ...
    for idx, row in enumerate(rows[1:], start=2):
        ...
        # 构建SQL - 使用text包装避免占位符问题
        columns = list(data.keys())
        values = list(data.values())
        
        sql = """
            INSERT INTO public.t_qs_hplc_reference ({})
            VALUES ({})
        """.format(
            ', '.join(columns),
        ...
        try:
            # psycopg2 requires dict for named parameters or positional with list
            # Build named parameter dict
            params = {columns[i]: values[i] for i in range(len(columns))}
            placeholders = ','.join([f"%({columns[i]})s" for i in range(len(columns))])
            sql = "INSERT INTO public.t_qs_hplc_reference ({}) VALUES ({})".format(
                ','.join(columns),
                placeholders
            )
