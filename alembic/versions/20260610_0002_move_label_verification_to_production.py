"""move label verification to production schema
...
def upgrade() -> None:
    # Drop old table from quality schema
    ...
    # Create new table in production schema
    op.create_table(
        "label_verifications",
        # 基础信息
    ...
    # 索引
    op.create_index("ix_label_verifications_batch_number", "label_verifications", ["batch_number"], schema="production")
    op.create_index("ix_label_verifications_production_date", "label_verifications", ["production_date"], schema="production")
    op.create_index("ix_label_verifications_verification_date", "label_verifications", ["verification_date"], schema="production")
    op.create_index("ix_label_verifications_result_status", "label_verifications", ["result_status"], schema="production")


def downgrade() -> None:
    op.drop_index("ix_label_verifications_result_status", table_name="label_verifications", schema="production")
    op.drop_index("ix_label_verifications_verification_date", table_name="label_verifications", schema="production")
    op.drop_index("ix_label_verifications_production_date", table_name="label_verifications", schema="production")
    op.drop_index("ix_label_verifications_batch_number", table_name="label_verifications", schema="production")
    op.drop_table("label_verifications", schema="production")

    # Recreate old table in quality schema
    op.create_table(
        "label_verifications",
        sa.Column("batch_number", sa.String(length=32), nullable=False),
        sa.Column("product_name", sa.String(length=128), nullable=False),
