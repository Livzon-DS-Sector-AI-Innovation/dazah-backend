...
def upgrade() -> None:
    conn = op.get_bind()
    # Only drop feishu_department_id if it exists
    result = conn.execute(sa.text(
        "SELECT column_name FROM information_schema.columns "
        "WHERE table_schema='identity' AND table_name='users' AND column_name='feishu_department_id'"
    ))
    if result.fetchone():
        op.drop_column('users', 'feishu_department_id', schema='identity')

    # Only add feishu_department_ids if it doesn't exist
    result = conn.execute(sa.text(
        "SELECT column_name FROM information_schema.columns "
        "WHERE table_schema='identity' AND table_name='users' AND column_name='feishu_department_ids'"
    ))
